<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="适用人群" prop="shiyongrenqun">
          <el-input v-model="ruleForm.shiyongrenqun" 
              placeholder="适用人群" clearable  :readonly="ro.shiyongrenqun"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="适用人群" prop="shiyongrenqun">
              <el-input v-model="ruleForm.shiyongrenqun" 
                placeholder="适用人群" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="帮助内容" prop="bangzhuneirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.bangzhuneirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.bangzhuneirong" label="帮助内容" prop="bangzhuneirong">
                    <span v-html="ruleForm.bangzhuneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.jiankangshizhanghao!=''&&json.jiankangshizhanghao){
                this.ruleForm.jiankangshizhanghao = json.jiankangshizhanghao
	    		this.ro.jiankangshizhanghao = true;
		}
        } else {
	...
	this.ruleForm.bangzhuneirong = this.ruleForm.bangzhuneirong.replace(reg,'../../../springboot0m6og/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiankangshouce/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiankangshouceCrossAddOrUpdateFlag = false;
						 this.parent.search();
