...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jiankangyinshi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','jiankangyinshi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('jiankangyinshi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjiankangyinshi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('jiankangyinshi', this.detail);
				this.$utils.msg('审核成功');
